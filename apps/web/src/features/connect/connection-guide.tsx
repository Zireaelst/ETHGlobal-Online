...
export function ConnectionGuide() {
  ...
  return <div className="connect-guide">
    ...
    <section className="client-connectors"><div className="connect-section-heading"><span className="app-kicker">MCP clients</span><h2>One server. Your preferred agent.</h2><p>Run these commands from the repository root after `pnpm agent:build`. Each client starts the same Node process over stdio.</p></div><div className="client-cards">{agentClients.map((client) => <article key={client.name}><div><span>{client.mark}</span><div><h3>{client.name}</h3><small>stdio · project local</small></div></div><div className="command-block"><pre><code>{client.command}</code></pre><CopyButton value={client.command} /></div><p>Check: <code>{client.check}</code></p></article>)}</div></section>
